...
class ChargingProfilesPanel(QWidget):
    ...
    def update_from_engine(self, engine: "Engine", bridge: "Bridge") -> None:
        ...
        manager = None
        if bridge.runner.adapter and hasattr(
            bridge.runner.adapter, "charging_profile_manager"
        ):
    ...
    def _create_limit_card(
        self, connector_id: int, max_current: float, effective_limit: Optional[float]
    ) -> QFrame:
        ...
        if effective_limit is not None:
            ...
            limit_label = QLabel(f"{get_icon_html('infinity', '#6e7681', 14)} No limit")
    ...
    def _update_profiles_list(self, profiles: list) -> None:
        ...
        sorted_profiles = sorted(
            profiles, key=lambda p: (p[0], p[1].charging_profile_id)
        )
    ...
    def _create_profile_card(self, connector_id: int, profile) -> QFrame:
        ...
        badge_text, badge_color = purpose_map.get(
            purpose_text, (purpose_text, "#6e7681")
        )

        purpose_label = QLabel(f"{get_icon_html('tag', badge_color, 12)} {badge_text}")
        ...
        kind_label = QLabel(f"{get_icon_html('clock', kind_color, 12)} {kind_badge}")
        ...
        if periods:
            ...
            for i, period in enumerate(periods[:max_show]):
                unit = (
                    "A"
                    if schedule.charging_rate_unit == ChargingRateUnitType.amps
                    else "W"
                )
