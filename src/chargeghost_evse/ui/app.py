...
from textual.widgets import Header, Footer, Button, Label, Input, TabbedContent, TabPane, Checkbox
...
class SimulatorScreen(Screen):
            ...
            with Vertical(id="sim-controls", classes="controls-panel"):
                ...
                with TabbedContent(id="main-tabs", initial="controls-tab"):
                    ...
                    with TabPane("Config", id="config-tab"):
                        ...
                        yield Input(placeholder="wss://localhost:3000/CP_1", id="input_url")
                        ...
                        yield Input(placeholder="Password", id="input_password", password=True)
    ...
    def _populate_config_fields(self) -> None:
        ...
        self.query_one("#input_connectors", Input).value = str(self.config.num_connectors)
        self.query_one("#checkbox_skip_tls", Checkbox).value = self.config.skip_tls_verify
    ...
    def action_save_config(self) -> None:
        ...
        self.config.charge_point_model = self.query_one("#input_model", Input).value or "ChargeGhostV1"
        self.config.charge_point_vendor = self.query_one("#input_vendor", Input).value or "ChargeGhost"
        try:
            self.config.num_connectors = int(self.query_one("#input_connectors", Input).value or "1")
        except ValueError:
            self.config.num_connectors = 1
        self.config.skip_tls_verify = self.query_one("#checkbox_skip_tls", Checkbox).value
        self.config.save()
        self.log_panel.log_message("[green]Config:[/green] Configuration saved. Restart to apply connection changes.")
