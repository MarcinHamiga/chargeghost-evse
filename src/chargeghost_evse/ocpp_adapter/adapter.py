...
class Adapter(cp):
    ...
    async def on_remote_start_transaction(
        ...
        **kwargs
        ...
        self._log(message=f"Received RemoteStartTransaction: connector_id={connector_id}, id_tag={id_tag}")
        ...
        if self.command_queue:
            self._log(message=f"Enqueuing START command for connector index: {internal_connector_index}")
    ...
    async def on_remote_stop_transaction(
        ...
        **kwargs
        ...
        self._log(message=f"Received RemoteStopTransaction: transaction_id={transaction_id}")
        ...
        if self.command_queue:
            self._log(message=f"Enqueuing STOP command for transaction: {transaction_id}")
            self.command_queue.put({
                "action": "STOP",
    ...
    async def send_boot_notification(self) -> call_result.BootNotification:
        ...
        self._log(message=f"Sending BootNotification: model={self.charge_point_model}, vendor={self.charge_point_vendor}")
        
        response: call_result.BootNotification = await self.call(request)
        self._log(message=f"BootNotification response: status={response.status}, interval={response.interval}")
        
        self.registration_status = response.status
        if response.status == RegistrationStatus.accepted:
            self.heartbeat_interval = response.interval
            self._log(message=f"Registration accepted. Heartbeat interval: {self.heartbeat_interval}s")
    ...
    async def send_authorize(self, id_tag: str) -> call_result.Authorize:
        ...
        status = response.id_tag_info.get("status", "Unknown") if response.id_tag_info else "Unknown"
    ...
    async def send_start_transaction(
        ...
        self._log(message=f"Sending StartTransaction: connector_id={connector_id}, id_tag={id_tag}")
        
        response: call_result.StartTransaction = await self.call(request)
        id_tag_status = response.id_tag_info.get("status", "Unknown") if response.id_tag_info else "Unknown"
        self._log(message=f"StartTransaction response: transaction_id={response.transaction_id}, status={id_tag_status}")
    ...
    async def send_stop_transaction(
        ...
        self._log(message=f"Sending StopTransaction: transaction_id={transaction_id}, meter_stop={meter_stop}")
        
        response: call_result.StopTransaction = await self.call(request)
        id_tag_status = response.id_tag_info.get("status", "No info") if response.id_tag_info else "No info"
    ...
    async def send_meter_values(
        ...
        self._log(message=f"Sending MeterValues: connector_id={connector_id}, value={value}Wh")
    ...
    async def send_status_notification(
        ...
        self._log(message=f"Sending StatusNotification: connector_id={connector_id}, status={status}")
