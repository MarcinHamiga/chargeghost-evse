...
class WebSocketManager:
    ...
    def subscribe_to_engine(self, engine: Any) -> None:
        ...
        self._unsubscribers = [
            engine.connector_status_changed.subscribe(
                self._on_connector_status_changed_ref
            ),
    ...
    async def _resolve_provider(
        self, provider: Optional[StateProvider]
    ) -> dict[str, Any]:
        if provider is None:
            return {}
        data = provider()
    ...
    async def disconnect(self, websocket: WebSocket) -> None:
        ...
        self._logger.info(
            "WebSocket client disconnected (total: %s)", len(self._clients)
        )
    ...
    def _schedule_broadcast(self, event_type: str, data: dict[str, Any]) -> None:
        if self._loop is not None and not self._loop.is_closed():
            asyncio.run_coroutine_threadsafe(
                self._broadcast(event_type, data), self._loop
            )
