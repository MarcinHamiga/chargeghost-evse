...
class TestV201AdapterStatusNotification:
	def test_status_notification_maps_available(self):
		...
		asyncio.run(adapter.send_status_notification(
...
class TestV201AdapterTransactionEvent:
	def test_transaction_event_started_creates_transaction(self):
		...
		assert request.transaction_info.transaction_id == "1"
	...
	def test_transaction_event_updated_increments_seq(self):
		...
		asyncio.run(adapter.send_transaction_event_started(
			connector_id=1, id_tag="TAG1", meter_start=0,
			timestamp="2026-01-01T00:00:00Z",
		))

		asyncio.run(adapter.send_transaction_event_updated(
			connector_id=1,
			transaction_id="1",
	...
	def test_transaction_event_ended_ends_transaction(self):
		...
		asyncio.run(adapter.send_transaction_event_ended(
			connector_id=1,
			transaction_id="1",
...
class TestV201AdapterMeterValues:
	def test_meter_values_sends_request(self):
		...
		asyncio.run(adapter.send_meter_values(
