...
class TestBootNotification:
	def test_boot_notification(self, client, app):
		...
		result = _make_ocpp_result(status="Accepted", current_time="2025-01-01T00:00:00Z", interval=300)
		with patch.object(
			type(app.state.runtime), "send_ocpp_raw", new_callable=AsyncMock, return_value=result
		):
...
class TestStatusNotification:
	def test_status_notification(self, client, app):
		_set_adapter_available(app)
		with patch.object(
			type(app.state.runtime), "send_ocpp_raw", new_callable=AsyncMock, return_value=None
		) as mock_send:
			response = client.post(
				"/api/v1/ocpp/status-notification",
				json={"connector_id": 1, "error_code": "NoError", "status": "Available"},
			)
		...
		mock_send.assert_called_once_with("send_status_notification", 1, "NoError", "Available")


class TestStartTransaction:
	def test_start_transaction(self, client, app):
		_set_adapter_available(app)
		result = _make_ocpp_result(transaction_id=42, id_tag_info={"status": "Accepted"})
		with patch.object(
			type(app.state.runtime), "send_ocpp_raw", new_callable=AsyncMock, return_value=result
		) as mock_send:
			response = client.post(
	...
	def test_data_transfer(self, client, app):
		...
		with patch.object(
			...
			response = client.post(
				...
				json={"vendor_id": "VendorX", "message_id": "GetInfo", "data": "payload"},
			)
		...
		assert data["details"]["status"] == "Accepted"
		mock_send.assert_called_once_with("send_data_transfer", "VendorX", "GetInfo", "payload")
